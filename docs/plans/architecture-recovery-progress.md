...
| 14 | [#167](https://github.com/justinrooks/arcus-signal/issues/167) | 13 | Clarify Storm Setup candidate attempts | 02, 09, 13 | `gpt-5.6-sol`, high | READY FOR COMMIT |
...
- **Files changed:** `Sources/App/StormSetup/StormSetupProvider.swift`, `Sources/App/StormSetup/StormSetupAnvilEvidencePolicy.swift`, `Tests/AppTests/StormSetupProviderTests.swift`, `Tests/AppTests/StormSetupAnvilEvidencePolicyTests.swift`, and this progress entry.
- **Behavior:** `DefaultStormSetupProvider` now represents each selected surface candidate, its source metadata, and its surface success or existing recorded/logged failures as one explicit attempt value. The provider still owns candidate iteration, cache access, surface acquisition, Anvil invocation, cancellation propagation, assessment, logging, fallback, and response composition. Exact/stale/unavailable Anvil timing and recognized stale-warning classification moved to one synchronous, side-effect-free policy. Candidate order, failure accumulation and status mapping, cache semantics, Anvil refresh, canonical values, `profileAnalysis`, response fields, and logging metadata are unchanged.
- **Coverage:** Added six direct deterministic policy cases for exact alignment, request/debug mismatch, recognized stale fallback with warning retention and floored age, unacknowledged older evidence, future evidence, and missing surface valid time. Provider coverage now also directly proves ordered multi-candidate failure accumulation and final HTTP status mapping, plus usable surface and exact Anvil response retention when snapshot-cache storage fails. Existing provider coverage continues to exercise cache-first refresh, candidate and normalization fallback, selected fallback alignment, cancellation, exact/stale/unavailable response behavior, and error mapping.
- **Validation:** Pre-change `swift test --filter StormSetupProviderTests` passed 18 tests. Final `StormSetupProviderTests` passed 20 tests, `StormSetupAnvilEvidencePolicyTests` passed 6, `StormSetupCurrentResponseDTOTests` passed 2, `StormSetupControllerTests` passed 6, `AnvilIngredientEvidenceTests` passed 4, `TornadoIngredientInterpreterTests` passed 42, and the review-requested `PressureArtifactDiagnosticsTests` passed 9. `swift build -Xswiftc -strict-concurrency=complete -Xswiftc -warn-concurrency` passed with only the pre-existing deprecated `ArcusEvent.title` warning. The scoped #167 whitespace check passed. Unscoped `git diff --check` remains blocked only by the pre-existing `docs/Sql/Device.sql:48` trailing whitespace.
- **Review:** Human review completed with no suggested changes. The fresh independent defect review returned GO with no actionable findings. The fresh test audit returned NO-GO on validation completeness only: it identified medium gaps for ordered exhausted-candidate failure/status coverage and explicit cache-store failure degradation coverage. Both findings were accepted and fixed with focused provider tests; re-audit returned GO. Focused defect re-review then identified a low test-safety issue in the new failure-count assertion; it was accepted and fixed with a gating `try #require`, and final defect re-review returned GO. No reviewer identified a disputed, architectural, contract-changing, or scope-expanding finding. Final full-diff review found no cache, candidate-order, fallback, cancellation, failure-mapping, Anvil I/O, evidence-policy, assessment, logging, canonical-value, `profileAnalysis`, or response-contract drift.
- **Residual risk / handoff:** The change is structural and adds no public contract, dependency, protocol, task, actor, queue, persistence, or filesystem behavior. The implementation and review lifecycle are complete and the issue is ready for commit. The pre-existing unrelated changes in `docs/Sql/Device.sql`, `docs/audits/weekly-bug-scan.md`, and `docs/audits/weekly-test-gap-audit.md` were preserved and excluded. No branch, commit, push, or pull request was created.
...
- **Files changed:** `Sources/App/StormSetup/GribAdapter.swift`, `Tests/AppTests/ProcessRunnerTests.swift`, `Tests/AppTests/Fixtures/ProcessRunnerFixture.py`, `Tests/AppTests/StormSetupWgrib2ClientTests.swift`, and this progress entry.
- **Behavior:** `ProcessRunner.run` now owns the launched direct child in the awaiting task. Nonblocking stdout/stderr readiness callbacks begin draining immediately after launch, and process termination is observed through one locked continuation. Parent cancellation initiates TERM, the operation asynchronously waits through the existing 0.5-second grace, escalates to SIGKILL when needed, observes the reaped child, awaits both readers, and then throws `CancellationError`. Timeout uses the same termination path and preserves its error shape and captured stderr; natural success, nonzero exit, launch failure, process arguments, and callers are unchanged. Cancellation observed before post-cleanup error mapping takes precedence over timeout or nonzero-exit mapping.
- **Coverage:** Moved process lifecycle characterization into a serialized `ProcessRunnerTests` suite backed by one deterministic Python fixture. Nine cases cover empty-stream success, exact nonempty success output, nonzero exit and stderr, timeout with TERM observation/SIGKILL/direct-PID reaping, simultaneous 15,000-line stdout/stderr drainage, TERM-cooperative cancellation, TERM-resistant cancellation with SIGKILL, cancellation before launch, and launch-failure mapping. Marker polling is bounded, cancellation tests enforce scheduler-tolerant grace bounds, and cancellation/timeout tests verify the exact recorded child PID no longer exists before completion.
- **Validation:** Pre-change `swift test --filter StormSetupWgrib2ClientTests` passed 9 tests. Final `swift test --filter ProcessRunner` passed 9 tests on three consecutive post-review runs; the independent defect reviewer also passed all lifecycle cases with `LIBDISPATCH_COOPERATIVE_POOL_STRICT=1`, including large dual-pipe drainage, timeout escalation, and TERM-resistant cancellation. `swift test --filter StormSetupWgrib2ClientTests` passed 5 tests; `swift test --filter PressureArtifactWarmJobTests` passed 18 tests; `swift test --filter StormSetupProviderTests` passed 20 tests; and `swift build -Xswiftc -strict-concurrency=complete -Xswiftc -warn-concurrency` passed. Test compilation emitted only pre-existing warnings. The issue-scoped whitespace check passed; unscoped `git diff --check` remains blocked only by the pre-existing `docs/Sql/Device.sql:48` trailing whitespace.
- **Manual runtime review:** Reviewed pre-launch cancellation, launch failure, natural exit, nonzero exit, timeout, graceful TERM cancellation, ignored TERM/SIGKILL cancellation, empty and large pipe drainage, reaping, callback/continuation synchronization, and cancellation/error-precedence paths. Every post-launch return or throw observes process exit and awaits both pipe readers; the cancellation handler only initiates termination, leaving one cleanup owner. No detached task, blocking cooperative-executor read/wait, custom `@unchecked Sendable`, actor, process group, registry, dependency, target, or caller contract was introduced.
- **Review:** Human review completed with no suggested changes. The independent defect review identified a high-severity cooperative-executor starvation defect in the original structured blocking pipe readers; it was accepted and fixed with callback/continuation-based pipe and exit observation, and constrained-executor re-review returned GO. The test audit identified a medium missing TERM-grace timing assertion; it was accepted and fixed. Focused re-audit then identified a medium empty-stream callback EOF gap; it was accepted and fixed with a deterministic empty-output case, and final re-audit returned GO. There were no disagreements, rejected findings, deferred actionable findings, architectural escalations, contract changes, or scope expansions. Final full-diff review found no lifecycle ownership, cancellation, timeout, error-mapping, output-capture, caller, weather, or planning-scope drift.
- **Residual risk / handoff:** Ownership remains intentionally limited to the direct child; arbitrary descendant-tree termination is out of scope and was not required by the fixture. The reviewers retained non-actionable watchlist notes for the tiny uninstrumented pre-launch race and an OS-level uninterruptible child; current implementation ordering covers the former and the latter is not fixture-testable. Live GitHub issue retrieval was unavailable because GitHub CLI and web access could not reach the private issue, so issue validation used the supplied prompt plus the checked-in epic/runbook/roadmap/progress evidence and the exact branch/base/prerequisite commits. The pre-existing unrelated changes in `docs/Sql/Device.sql`, `docs/audits/weekly-bug-scan.md`, and `docs/audits/weekly-test-gap-audit.md` were preserved and excluded. No commit, push, or pull request was created. Ready for commit.
...
- **Files changed:** `Tests/AppTests/IntegrationTestSupport.swift`, `Tests/AppTests/NotificationSendJobDeliveryBoundaryTests.swift`, `Tests/AppTests/NotificationSendJobCandidateQueryTests.swift`, `Tests/AppTests/NotificationMissedDecisionPersistenceTests.swift`, `Tests/AppTests/NotificationLedgerFreshnessPersistenceTests.swift`, `Tests/AppTests/NWSIngestPersistenceFlowTests.swift`, `Tests/AppTests/NWSAlertLifecycleTests.swift`, and this progress entry.
- **Behavior:** Added one feature-neutral integration-test lifecycle seam for `.testing` application creation and one asynchronous shutdown, canonical direct-Postgres URL resolution/registration, configured application setup with optional migration, suite-local preparation, and rollback-after-assertions transactions. The four notification suites retain direct Postgres setup and their local bootstrap SQL; the NWS flow and lifecycle suites retain API configuration and migrations. Scenario fixtures, assertions, cleanup, serial annotations, and the forced-late-failure rollback sentinel remain local and unchanged.
- **Validation:** Pre-change `swift test --filter Notification` and `swift test --filter NWS` passed. Post-change `swift test --filter Notification` passed 30 tests in 7 suites; `swift test --filter NWS` passed 11 tests in 3 suites; `swift test --filter NotificationSendJobDeliveryBoundaryTests` passed 6 tests; `swift test --filter NotificationSendJobCandidateQueryTests` passed 3 tests; and `swift test --filter NWSIngestPersistenceFlowTests` passed 5 tests. `swift test --no-parallel` passed 470 tests in 61 suites; `swift test --parallel --num-workers 8` passed 470 tests in 61 suites, with no migrated-suite interference. Existing compiler warnings remain outside this slice.
- **Review:** Human review completed with no suggested changes. Implementation self-review and independent defect/test audits returned GO with no findings or disagreements: the helper remains feature-neutral; lifecycle/error precedence, direct Postgres setup, configured migrations, suite-local preparation, rollback semantics, scenario bodies, SQL, serialization, cleanup, and the forced-late-failure path remain preserved. Final focused review validation passed 3 candidate-query tests and 5 NWS persistence-flow tests. No production file, migration, pressure gate, global lock, fixture DSL, new dependency, or test target was added. GitHub API retrieval was unavailable in this environment, so live issue and epic content could not be read; validation used the supplied issue prompt plus the checked-in runbook, roadmap, progress ledger, expected base, and prerequisite commits. The unscoped whitespace check reports only the pre-existing `docs/Sql/Device.sql:48` trailing whitespace; the issue-scoped check passes. The protected unrelated documentation changes remain untouched. No commit, push, or pull request was created. Ready for commit.
